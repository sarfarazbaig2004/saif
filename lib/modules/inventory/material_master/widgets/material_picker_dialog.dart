...
class _MaterialPickerDialogState extends State<MaterialPickerDialog> {
  ...
  Future<void> _load([String query = '']) async {
    ...
    try {
      ...
      final deduped = <String, MaterialMasterModel>{};

      for (final material in materials) {
        final key = material.normalizedMaterialCode.trim();

        if (key.isEmpty) continue;

        if (!deduped.containsKey(key)) {
          deduped[key] = material;
          continue;
        }

        final existing = deduped[key]!;

        if (!existing.isActive && material.isActive) {
          deduped[key] = material;
        }
      }
      ...
      debugPrint(
        ...
        'count=${materials.length} deduped=${dedupedList.length}',
      );
